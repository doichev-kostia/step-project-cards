...
export default function createHeaderSection() {
    ...
    function createLoginButton(parent) {
        ...
        createdButton.addEventListener("click", event => {
            const modal = new ModalLogin(root, 'Вход', {modalWrapper:'modalWrapper',modal:'modal',crossButton: 'cross',title:'modal-title',loginButton:'btn'})
            modal.render()
        })
    }
    ...
    function test(){
        const modal = new ModalCreateVisit(TODELETE,"Создать визит",{modalWrapper:'modalWrapper',modal:'modal',crossButton: 'cross',title:'visit-title',loginButton:'btn'}).render()
    ...
    function deleteDoctorSet(parent, doctor){
        ...
        // if (toDelete) toDelete.forEach(elem => elem.remove());
    }
